...
func TestHandshakeKyber(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectKyber {
				if ss.testingOnlyPeerSignatureAlgorithm != x25519Kyber768Draft00 {
					t.Errorf("got CurveID %v (server), expected %v", ss.testingOnlyPeerSignatureAlgorithm, x25519Kyber768Draft00)
				}
				if cs.testingOnlyPeerSignatureAlgorithm != x25519Kyber768Draft00 {
					t.Errorf("got CurveID %v (client), expected %v", cs.testingOnlyPeerSignatureAlgorithm, x25519Kyber768Draft00)
				}
			} else {
				if ss.testingOnlyPeerSignatureAlgorithm == x25519Kyber768Draft00 {
					t.Errorf("got CurveID %v (server), expected not Kyber", ss.testingOnlyPeerSignatureAlgorithm)
				}
				if cs.testingOnlyPeerSignatureAlgorithm == x25519Kyber768Draft00 {
					t.Errorf("got CurveID %v (client), expected not Kyber", cs.testingOnlyPeerSignatureAlgorithm)
				}
			}
			if test.expectHRR {
